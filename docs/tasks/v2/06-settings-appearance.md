...
状态：In progress（体验版已接入，分支 feat/v2-01；P-V2-05/06/07 待用户体验后决定，见下文“体验版接入”）。2026-09-23 新增（V2-D20）。依赖：V2-01（前端 store 与单编辑器实例）；排在 V2-01 之后、V2-02 之前（V2-D26）。开工前需要确定 P-V2-05（diff 颜色语义）、P-V2-06（移植范围）、P-V2-07（默认配色）。
...
未完成（不能勾选 B19–B22）：P-V2-05/06/07 未决定；禁止写死颜色的静态检查；B22 许可声明；常驻 cat-file 在改 Git 路径后通过重新载入项目换新路径（体验版做法），未做空闲回收后的平滑切换。
