...
export default function SpecialFileView({ description, compact = false, action }: { description: SpecialDescription; compact?: boolean; action?: { label: string; onClick(): void } }) {
  return <section className={`special-file ${description.kind}${compact ? " compact" : ""}`} role="status" aria-label={description.title}>
    ...
    {action && <button type="button" className="special-file-action" onClick={action.onClick}>{action.label}</button>}
  </section>;
}
