...
export default function App() {
  ...
  useEffect(() => { setRefsView(null); setFileHistoryRequest(null); setBranchOpen(false); setStashCount(null); setSyncOpen(false); }, [activeRepoId]);
  ...
  const runPull = async (initial: "ffOnly" | "merge") => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (confirmation.reason === "diverged" && request.mode === "ffOnly") {
        ...
        const ok = await askConfirm({ title: "stash 后拉取", message: confirmation.message, items: confirmation.paths, notes: [untracked ? "将储藏全部本地改动，包含未跟踪文件" : "将储藏已跟踪文件的全部改动（不含未跟踪文件）", "储藏为新的 stash@{0}；拉取后不会自动恢复，之后可在底部“Stash”页应用或弹出"], confirmLabel: "stash 后拉取" });
        if (!ok) return;
  ...
  return <main className="app">
    ...
    <GitPanel repoId={activeRepoId} tab={gitTab} onTab={setGitTab} stagedCount={stagedCount} headOid={runtime?.snapshot?.branchInfo?.oid ?? null} headKey={`${snapshot?.branchInfo?.oid ?? ""}:${snapshot?.branchInfo?.upstream ?? ""}:${snapshot?.branchInfo?.ahead ?? ""}:${snapshot?.branchInfo?.behind ?? ""}`}
      ...
      logContent={activeRepoId && logMounted === activeRepoId ? <HistoryPanel key={activeRepoId} repoId={activeRepoId} refsVersion={refsVersion} hidden={gitTab !== "log"} fileHistoryRequest={fileHistoryRequest} activeKey={historyReading?.key ?? null} onOpenFile={(open) => void openHistoryFile(open)} onFetch={openFetch} fetchBlocked={writeBlocked ?? (noRemote ? "该仓库没有配置 remote；Oris 不会新增 remote" : null)} fetchText={fetchText} onRefs={setRefsView}
        writeBlocked={writeBlocked} onCheckout={(oid) => void runSwitch({ kind: "checkout", commit: oid })} onNewBranch={(start) => { loadRefsView(); setNewBranch({ initial: start }); }} onMerge={detachedOid ? undefined : startMerge}/> : null}
