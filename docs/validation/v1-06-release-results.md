...
| B12 | 部分通过；真实远端未运行 | 本地：v2-04 B12 5/5（进度、取消结束进程树、无输出超时、认证失败不弹凭据输入且脱敏）；后端 `b12_*` 2 项。**AgentHub（SSH / HTTPS）真实远端回归未运行**：从 Bash 工具启动测试实例的命令被本会话的自动权限分类器拦截；未改用其他方式绕过，远端引用未被改动（开工前 SSH `ls-remote` 只有 `HEAD`、`refs/heads/main`）。上一次真实远端通过记录见 [V2-04 结果](v2-04-results.md)（旧构建，不作本构建证据） |
...
- **AgentHub 真实远端（B12）**：本轮未运行（见追溯矩阵 B12 行）；需要用户在会话权限中放行“从 Bash 工具启动测试实例并推送 `oris-test/20260926-1014/` 测试分支”，或自行运行 `node scripts/perf/v2-04-acceptance.mjs --exe <oris.exe> --only real --run-id <运行编号>`。
