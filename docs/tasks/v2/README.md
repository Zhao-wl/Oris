...
| [V2-06 设置与外观](06-settings-appearance.md) | 设置框架与分类 → Git / 字号 / 浅深色迁入 → 颜色变量化 → VS Code 配色方案移植 | V2-01；排期待 P-V2-08 | Pending |

## 执行顺序（V2-D17，用户已确认）

V1 03 → **V2-01** → V2-02 → V1 04 → V2-03 → V2-04 → V1 05 → V2-05 → V1 06 合并发布验收（V2-D19：一二期合并为一次发布，一期不单独发布）
...
- V2-06（2026-09-23 新增，V2-D20）尚未排入上面的顺序。建议插在 V2-01 之后、V2-02 之前，待用户确认（P-V2-08）。
...
- V2-02 的 UI 前置条件已满足：用户已确认[一期与二期混合发布参考图](../../design/04-mixed-release-ui-reference.md)（V2-D18）。这不改变 V2-01 等任务依赖，也不表示 V2-02 已开工。
